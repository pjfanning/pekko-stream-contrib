...
object Retry {
  ...
  private[pekko] class RetryCoordinator[I, S, O](retryWith: S => Option[(I, S)])
  ...
  private[pekko] class RetryConcatCoordinator[I, S, O](retriesLimit: Long,
    ...
    override def createLogic(attributes: Attributes) = new GraphStageLogic(shape) {
      ...
      lazy val timeout =
        Duration.fromNanos(ConfigFactory.load().getDuration("org.apache.pekko.stream.contrib.retry-timeout").toNanos)
